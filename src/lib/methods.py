...
class Figure():
    ...
    def plt_spectrogram(self, 
                        data, t_data, parameter_data=None, 
                        figsize=(10, 5), xlabel='Parameter '+r'$\mu$', ylabel='Frequency [Hz]',
                        nperseg=2000, cmaplim=(-220, 80), cmap='jet', 
                        xlim=(None, None), ylim=(0, 500), 
                        save_filename=None
                        ):
        ...
        ax.pcolormesh(p, freq, db_plt, cmap=cmap)
        ax.set_xlim(xlim)
        ax.set_ylim(ylim)
    ...
    def plt_attractor_of_timedelay_embedding(
                                            self,
                                            data, n_plot=None, n_shift=10,
                                            figsize=(4, 4), title=None, linestyle='-', c='k', lw=2,
        ...
        ax1.plot(data[n_shift:][:n_plot], data[:-n_shift][:n_plot], linestyle=linestyle, c=c, lw=lw)
    ...
    def plt_bifurcation_in_descrete(self, 
                                    data, parameter_data, parameter_lim=(None, None),
                                    figsize=(10, 5), xlabel='Parameter '+r'$\mu$', ylabel=r'$x$',
                                    marker='.', s=50, c='b', 
                                    save_filename=None):
        fig = plt.figure(figsize=figsize)
        ax = fig.add_subplot(111)
        for i in range(len(data)):  
            ax.scatter(np.full(data[i].shape, parameter_data[i]), data[i], marker=marker, c=c, s=s)
        ax.set_xlim(parameter_lim)
        ax.set_xlabel(xlabel)
        ax.set_ylabel(ylabel)
        plt.tight_layout()
        if save_filename==None:
    ...
    def plt_2powerspectra(self, 
        ...
        ax.legend(loc='center left', bbox_to_anchor=legend_xy)
        plt.tight_layout()
        if save_filename==None:
            plt.show()
        else:
